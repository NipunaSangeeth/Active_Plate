...
function TheCombined() {
  ...
  /**
   * Submit Combined Plan data to Firestore "theCombinedPlans" Collection
   * Performs basic validation before saving asynchronously.
   */
  const handlesubmitClick = async () => {
    // Validate ALL fields are filled
    if (!gender || !userAge || !userWeight || !userExperience || !userIllnesses || !userFitness || !userlocation || !userMotivation || !userTime || !userStress || !userInjury || !userHealth || !userContact || !userSleep_Pattern || !userMedication || !userDisability) {
    ...
    try {
      await addDoc(collection(db, "theCombinedPlans"), {
        gender,
        userAge,
        userWeight,
        userExperience,
      ...
      dispatch(alertSuccess("Combined Plan submitted successfully!"));
      setTimeout(() => {
        dispatch(alertNull());
        navigate("/report", { 
          state: { 
            serviceType: "TheCombined", 
            formData: { gender, userAge, userWeight, userExperience, userIllnesses, userFitness, userlocation, userMotivation, userTime, userStress, userInjury, userHealth, userContact, userSleep_Pattern, userMedication, userDisability } 
          } 
        });
      }, 2000);
    } catch (error) {
